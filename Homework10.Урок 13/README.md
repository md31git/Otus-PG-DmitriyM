...
Вместо виртуальных машин буду использовать разные кластеры PG на одной ВМ. Для меня как разработчика БД развертывание нескольких ВМ и настройка доступа между ними является не тривиальной задачей и обычно приводить к "танцы с бубном" (опыт из предыдущих ДЗ). Право на развертывание ВМ оставим Администраторам БД, с которыми я в будущем планирую работать. Для понимания репликации и ее настройки достаточно работы с несколькими кластерами. Поэтому было принято такое решение. 

## 1. Создание 4-х кластеров:

Первый кластер PG уже развернут (порт 5432, директория main):

![image](https://github.com/user-attachments/assets/8d41a60a-e964-403f-84fa-639b41bb6fd7)

Остальные разворачиваем командами 
```bash
...
1. порт 5432, директория main
2. порт 5433, директория main2
3. порт 5434, директория main3
4. порт 5435, директория main4
...
```
Создаем базу db и схему test в ней и 2 таблицы в test (заполненную данными) и test2:
```bash
CREATE Database db;
\c db
CREATE schema test;
...
```
Cоздаем базу db и схему test в ней и 2 таблицы в test и test2(заполненную данными):
```bash
CREATE Database db;
\c db
CREATE schema test;
...
Но на кластере 1 уже установлено значение то что нам нужно. Поэтому ничего не меняем.
```bash
sudo -u postgres psql -d postgres -p 5432
show wal_level;
exit
...
## 5.Кластер 3 использовать как реплику для чтения и бэкапов (подписаться на таблицы из Кластера 1 и 2 ).
...
Создаем подписки на таблицы с первоначальным копированием данных (copy_data = true) и проверяем что они создались и данные скопированы:
```bash
...
Затем останавливаем Кластер 4 и удаляем его содержимое, чтобы можно было восстановить туда данные из Кластера 3 и включить физическую репликацию.
```bash
...
Пришлось дать права владельца на директорию /var/lib/postgresql/14 пользователю postgres, иначе у него не было прав там создавать новую директорию.
