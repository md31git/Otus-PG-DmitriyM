...
Чтобы воспроизвети deadlock необходимо "зациклить" обращение например к строкам одной таблицы. Для этого откроем 3 сеанса и 3 транзакции и последовательно будем выполнять команды
...
Две в стостояении active - они ожидают другую транзакцию. А третья в состоянии idle in transaction - т.е. они ничего не делает и никого (другой ресурс) не ждет. 
...
Есть предположение, что при одновремнном изменении двух таблиц по всем записям произойдет эсколация блокировок с версии строк до таблицы одновремнно в двух транзакциях. И они друг друга заблокируют.
