...
### Открываем третью сессию для удобства и запрашиваем последние 10 строк из лог файла Postgresql. 
**3 сессия**
```Bash
...
3.Эксклюзивная блокировка виртуального номера транзакции virtualxid. Открывается когда открыта транзакция но ни одна команда еще не выполнена в ней, т.е. после слова BEGIN;
...
Все последующие сеансы будут накладывать аналогичные блокировки что и в 3 сеансе.
...
Чтобы воспроизвести deadlock необходимо "зациклить" обращение например к строкам одной таблицы. Для этого откроем 3 сеанса и 3 транзакции и последовательно будем выполнять команды
### 3.1. Сессия 1. Открываем транзакцию и обновляем строку с id=1
```Bash
begin;
update test.test set name='тест1' where id=1;
```
### 3.2. Сессия 2. Открываем транзакцию и обновляем строку с id=2
```Bash
begin;
update test.test set name='тест2' where id=2;
```
### 3.3. Сессия 3. Открываем транзакцию и обновляем строку с id=3
```Bash
begin;
update test.test set name='тест3' where id=3;
```
### 3.4. Сессия 1. Обновляем строку с id=2 и она становиться в ожидании(блокировка) сессии 2
```Bash
update test.test set name='тест1' where id=2;
```
### 3.5. Сессия 2. Обновляем строку с id=3 и она становиться в ожидании(блокировка) сессии 3
```Bash
update test.test set name='тест2' where id=3;
```
Перед возникновением deadlock состояние транзакций следующее:
![image](https://github.com/user-attachments/assets/92352d7a-47c1-45c4-b465-068c82ade7c7)
Две в состоянии active - они ожидают другую транзакцию. А третья в состоянии idle in transaction - т.е. они ничего не делает и никого (другой ресурс) не ждет. 
### 3.6. Сессия 3. Обновляем строку с id=1 и возникает взаимоблокировка 
```Bash
update test.test set name='тест3' where id=1;
```
![image](https://github.com/user-attachments/assets/a2f64f55-c41c-45a3-94cc-f7a64b2b8478)
При выводе ошибки deadlock указываются только номера транзакций и сессий, которые стали причиной этой ошибки.

### 3.7. Сессия 4. Чтобы посмотреть подробности по ошибке deadlock нужно открывать в log PostgreSQL. Такие ошибки туда пишутся автоматически.
```Bash
sudo tail -n 20 /var/log/postgresql/postgresql-14-main3.log
```
![image](https://github.com/user-attachments/assets/7525e7dc-5d34-42e1-bf6d-7a70d59c1fcf)
В принипе в простых ситуациях разобраться можно, но при реальных сложных и вложенных операциях будет очень сложно. 

## 4.Могут ли две транзакции, выполняющие единственную команду UPDATE одной и той же таблицы (без where), заблокировать друг друга?
Есть предположение, что при одновременном изменении двух таблиц по всем записям произойдет эскалация блокировок с версии строк до таблицы одновремнно в двух транзакциях. И они друг друга заблокируют.
Важно условие что таблицы должны быть большие: десятки миллионов строк, а также наличие индексов на изменяемый столбец. 
